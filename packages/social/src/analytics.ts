...
export function analyzeWeeklySocialMetrics(posts: readonly SocialPostMetric[]): WeeklySocialAnalysis | null {
  ...
  const windowScores = groupedMean(posts, (post) => postingWindow(post.publishedAt))
    .sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]));
  const rankedPosts = [...posts].sort((left, right) => engagementRate(right) - engagementRate(left) || right.reach - left.reach || left.id.localeCompare(right.id));
  ...
  const highestPotentialChange = `Prioritize ${bestFormat} posts in ${bestPostingWindow} and test hooks modeled on "${bestHook}".`;
