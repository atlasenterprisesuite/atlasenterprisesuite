...
function positiveValues(spec: ProductionSpec): Set<string> {
  ...
  for (const subject of spec.subjects) values.push(subject.label, subject.description, ...subject.appearanceTraits, ...subject.materialTraits, ...subject.allowedTransformations);
  for (const scene of spec.scenes) {
    values.push(scene.title, scene.description);
    for (const shot of scene.shots) values.push(
...
function orderedShots(spec: ProductionSpec): ShotSpec[] {
  return [...spec.scenes].sort((a, b) => a.startSecond - b.startSecond).flatMap(scene => [...scene.shots].sort((a, b) => a.order - b.order));
}

function hasAllowedTransformationNote(shot: ShotSpec, allowed: string[]) {
  const notes = shot.continuityNotes.map(normalized);
  return allowed.some(value => { const target = normalized(value); return target && notes.some(note => note.includes(target)); });
}
  ...
  for (const rule of spec.continuityRules) {
    ...
    for (const shot of shots.slice(start, end + 1)) {
      if (!shot.subjectIds.includes(subject.id) && !hasAllowedTransformationNote(shot, subject.allowedTransformations)) {
        issues.push(issue('identity_subject_missing', rule.severity, 'continuity', 'Identity-locked subject disappears inside an active continuity span without an allowed transformation note.', shot.id));
      }
    }
  }
}
...
export function validateProductionSpec(spec: ProductionSpec, provider?: ProviderCapability): ValidationResult {
  ...
  for (const scene of spec.scenes) {
    ...
    for (let index = 0; index < shots.length; index += 1) {
      ...
      if (previous && shot.startSecond < previous.endSecond) issues.push(issue('shot_overlap', 'blocking', 'shots', 'Shots may not overlap within a scene.', shot.id));
      if (previous && shot.transitionIn === 'continuous' && previous.camera.orientationRule.trim() && shot.camera.orientationRule.trim() && normalized(previous.camera.orientationRule) !== normalized(shot.camera.orientationRule)) {
  ...
  if (spec.motionComposition) {
    for (const motionIssue of validateMotionComposition(spec.motionComposition)) {
      issues.push(issue(motionIssue.code, motionIssue.severity, 'review', motionIssue.message, motionIssue.targetId));
    }
    if (spec.durationSeconds > 0 && spec.motionComposition.durationSeconds !== spec.durationSeconds) {
      issues.push(issue('motion.duration_mismatch', 'blocking', 'review', 'Motion composition duration must match production duration.', spec.motionComposition.id));
    }
  }
  ...
  for (const constraint of spec.negativeConstraints) {
    ...
    if (value && positives.has(value)) issues.push(issue('negative_constraint_conflict', 'blocking', 'continuity', 'A blocking negative constraint exactly conflicts with a positive production field.', constraint.id));
  }

  if (provider) {
    if (provider.connectionState !== 'ready') issues.push(issue('provider_not_ready', 'blocking', 'provider', 'Selected provider is not verified ready.'));
    if (provider.minDurationSeconds !== null && spec.durationSeconds < provider.minDurationSeconds) issues.push(issue('provider_duration_unsupported', 'blocking', 'provider', 'Requested duration is below provider capability.'));
    if (provider.maxDurationSeconds !== null && spec.durationSeconds > provider.maxDurationSeconds) issues.push(issue('provider_duration_unsupported', 'blocking', 'provider', 'Requested duration exceeds provider capability.'));
    if (!provider.aspectRatios.includes(spec.aspectRatio) && spec.aspectRatio !== 'adaptive') issues.push(issue('provider_aspect_ratio_unsupported', 'blocking', 'provider', 'Requested aspect ratio is not supported.'));
    if (spec.resolutionPreference !== 'adaptive' && !provider.resolutions.includes(spec.resolutionPreference)) issues.push(issue('provider_resolution_unsupported', 'blocking', 'provider', 'Requested resolution is not supported.'));
    if (spec.audioEnabled && !provider.audioSupport) issues.push(issue('provider_audio_unsupported', 'warning', 'provider', 'Provider does not support native synchronized audio.'));
    const references = countReferences(spec);
    if (provider.maxImageReferences !== null && references.image > provider.maxImageReferences) issues.push(issue('provider_image_reference_limit', 'blocking', 'provider', 'Image reference count exceeds provider capability.'));
    if (provider.maxVideoReferences !== null && references.video > provider.maxVideoReferences) issues.push(issue('provider_video_reference_limit', 'blocking', 'provider', 'Video reference count exceeds provider capability.'));
    if (provider.maxAudioReferences !== null && references.audio > provider.maxAudioReferences) issues.push(issue('provider_audio_reference_limit', 'blocking', 'provider', 'Audio reference count exceeds provider capability.'));
  }

  const status: ValidationResult['status'] = issues.some(item => item.severity === 'blocking') ? 'blocking' : issues.length > 0 ? 'warning' : 'pass';
