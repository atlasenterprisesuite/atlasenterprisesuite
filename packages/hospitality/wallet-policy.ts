import type { HospitalityCapability, HospitalityStayStatus, WalletPlatform } from './types';
...
function supportsWalletPlatform(platform: WalletPlatform, capabilities: readonly HospitalityCapability[]) {
  if (platform === 'apple_wallet') return capabilities.includes('wallet.apple.issue') && capabilities.includes('wallet.apple.provision');
  if (platform === 'google_wallet') return capabilities.includes('wallet.google.issue') && capabilities.includes('wallet.google.provision');
  if (platform === 'provider_app') return capabilities.includes('mobile_key.issue');
  return false;
}
...
export function evaluateWalletEligibility(input: WalletEligibilityInput): WalletEligibilityDecision {
  ...
  if (!supportsWalletPlatform(input.requestedPlatform, input.providerCapabilities)) return blocked('wallet_platform_not_supported');
  if (input.hasEquivalentActiveCredential && !input.replacementFlow) return blocked('active_credential_exists');

  return { eligible: true, blocker: null, replacementRequired, deliveryReady: input.supportedDeliveryPath };
}
...
export function buildHospitalityIdempotencyKey(input: {
  ...
  const components = [input.organizationId, input.propertyId, input.providerInstanceId, input.sourceEventId, input.sourceVersion];
  if (components.some((value) => value.trim().length === 0)) throw new Error('idempotency_component_required');
