...
async function probe(url: string) {
  ...
  try {
    const r = await timeout(url, { redirect: 'manual', headers: { 'user-agent': 'ATLAS-Manager/3.0' } });
    return { reachable: r.status >= 200 && r.status < 500, status_code: r.status, duration_ms: Date.now() - started };
  } catch (error) {
...
async function vercelState() {
  ...
  try {
    ...
    const r = await timeout(`https://api.vercel.com/v9/projects${qs}`, { headers: { authorization: `Bearer ${token}` } });
    const d = await r.json().catch(() => ({}));
    if (!r.ok) return { state: 'authorization_error', authorization: 'present', project_verified: false, status_code: r.status };
    const projects = Array.isArray(d?.projects) ? d.projects : [];
    const verified = configuredProject ? projects.some((p: any) => p?.id === configuredProject || p?.name === configuredProject) : projects.length > 0;
...
async function githubState() {
  ...
  const bridge = await probe(`${SUPABASE_URL}/functions/v1/atlas-repair-bridge?api=readiness`);
  if (!token) return { state: bridge.reachable ? 'oidc_bridge_ready_token_not_present' : 'authorization_unverified', repository: CANONICAL_REPO, repair_bridge: bridge };
  try {
    const r = await timeout(`https://api.github.com/repos/${CANONICAL_REPO}`, { headers: { authorization: `Bearer ${token}`, accept: 'application/vnd.github+json', 'x-github-api-version': '2022-11-28' } });
    if (!r.ok) return { state: r.status === 404 ? 'repository_not_visible' : 'authorization_error', repository: CANONICAL_REPO, status_code: r.status, repair_bridge: bridge };
    const repo = await r.json();
    const wr = await timeout(`https://api.github.com/repos/${CANONICAL_REPO}/actions/runs?branch=main&per_page=1`, { headers: { authorization: `Bearer ${token}`, accept: 'application/vnd.github+json', 'x-github-api-version': '2022-11-28' } });
    const wd = await wr.json().catch(() => ({}));
    const latest = Array.isArray(wd?.workflow_runs) ? wd.workflow_runs[0] : null;
    return { state: 'ready', repository: CANONICAL_REPO, default_branch: repo?.default_branch || 'main', latest_workflow: latest ? { status: latest.status, conclusion: latest.conclusion, head_sha: latest.head_sha, updated_at: latest.updated_at } : null, repair_bridge: bridge };
  } catch {
    return { state: 'unreachable', repository: CANONICAL_REPO, repair_bridge: bridge };
  }
}
...
async function cloudflareState() {
  ...
  try {
    ...
    const r = await timeout(target, { headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' } });
    const d = await r.json().catch(() => ({}));
    return { state: r.ok && d?.success !== false ? 'ready' : 'authorization_error', authorization: r.ok ? 'verified' : 'rejected', zone_verified: Boolean(zoneId && r.ok), status_code: r.status, public_edge: publicEdge };
  } catch {
...
Deno.serve(async (req: Request) => {
  ...
  if (github.state !== 'ready' && github.state !== 'oidc_bridge_ready_token_not_present') blockers.push({ stage: 'github', code: github.state, detail: 'Canonical repository health is not fully verified.' });
  ...
  const readiness = blockers.length === 0 ? 'ready' : blockers.some((b) => ['production', 'runtime', 'deployment'].includes(b.stage)) ? 'blocked' : 'partial';
