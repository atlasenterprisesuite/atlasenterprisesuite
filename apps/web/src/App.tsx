...
function EnterpriseHome() {
  return (
    <section className="page-stack">
      <PageHeader eyebrow="ATLAS Enterprise Suite" title="One governed ecosystem" description="Enterprise foundations for connected business and health modules, built with explicit security, evidence, and production gates." />
      <div className="hero-grid">
        <article className="feature-card accent"><p className="eyebrow">Active foundation</p><h2>ATLAS Health</h2><p>Health research architecture with explicit separation between evidence, hypothesis, contradiction and production state.</p><Link className="action-link" to="/health">Open Health</Link></article>
        <article className="feature-card"><p className="eyebrow">Environment</p><h2>Development</h2><p>No live clinical integrations, patient data, or production datastore are represented in this repository.</p></article>
      </div>
    </section>
  );
}
...
function HealthHome() {
  return (
    <section className="page-stack">
      <PageHeader eyebrow="Health" title="ATLAS Health" description="A governed smart-health and biomedical research ecosystem. This milestone implements the research layer without pretending to be a clinical system." />
      <ResearchBadge />
      <div className="module-grid">
        <Link className="module-card enabled" to="/health/research"><span>Research & Innovation</span><strong>Health Frontiers</strong><p>Evidence registry, Neural Graph, falsification, reconstruction vulnerabilities and curability classification.</p></Link>
        <div className="module-card disabled" aria-disabled="true"><span>Clinical systems</span><strong>Not configured</strong><p>EHR, FHIR, HL7 and patient workflows are intentionally absent from this milestone.</p></div>
        <div className="module-card disabled" aria-disabled="true"><span>Hospital operations</span><strong>No live connection</strong><p>No fabricated census, bed, staffing, pharmacy or facility metrics are displayed.</p></div>
      </div>
    </section>
  );
}
  ...
  return (
    <section className="page-stack">
      ...
      <Link className="feature-card link-card" to="/health/research/frontiers">
        <p className="eyebrow">Research program</p><h2>Health Frontiers</h2><p>Cross-disease investigation with explicit falsification and evidence-level controls.</p><span className="action-link">Enter Health Frontiers</span>
      </Link>
    </section>
  );
}
...
function FrontiersHome() {
  return (
    <section className="page-stack">
      <PageHeader eyebrow="Research & Innovation" title="Health Frontiers" description="A structured research workspace for persistent diseases, causal mechanisms, escape routes, repair and durable surveillance." />
      <ResearchBadge />
      <Link className="feature-card link-card accent" to={labBase}>
        <p className="eyebrow">Core laboratory</p><h2>Disease Reconstruction Lab</h2><p>Ask what minimum surviving biological states are sufficient to reconstruct disease, then test those connections against evidence and counterevidence.</p><span className="action-link">Open Disease Reconstruction Lab</span>
      </Link>
    </section>
  );
}
...
function LabOverview() {
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="Health Frontiers" title="Disease Reconstruction Lab" description="Map disease persistence as seed, state, niche, adaptation, reconstruction and relapse while preserving uncertainty and contradictory evidence." />
      <ResearchBadge />
      ...
      <div className="module-grid compact">
        ...
        <Link className="module-card enabled" to={`${labBase}/evidence`}><span>Provenance</span><strong>Evidence Registry</strong><p>Separate human, preclinical, mechanistic and hypothesis-level findings.</p></Link>
        <Link className="module-card enabled" to={`${labBase}/falsification`}><span>Challenge</span><strong>Falsification Engine</strong><p>Preserve counterexamples, escape routes, negative evidence and uncertainty.</p></Link>
        <Link className="module-card enabled" to={`${labBase}/vulnerability`}><span>Research model</span><strong>Vulnerability Engine</strong><p>Transparent, research-only reconstruction scoring with visible contributions.</p></Link>
      </div>
    </div>
  );
}
...
function EvidencePage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="Disease Reconstruction Lab" title="Evidence Registry" description="Every record exposes provenance, evidence level, limitations, replication state, confidence and scientific status." />
      <div className="filter-row">
        <label className="field">Evidence level<select value={level} onChange={(event) => setLevel(event.target.value as 'all' | EvidenceLevel)}><option value="all">All levels</option><option value="human_randomized">Human randomized</option><option value="human_interventional">Human interventional</option><option value="human_observational">Human observational</option><option value="human_case_report">Human case report</option><option value="preclinical_animal">Preclinical animal</option><option value="mechanistic">Mechanistic</option><option value="hypothesis">Hypothesis</option></select></label>
        <label className="field">Status<select value={status} onChange={(event) => setStatus(event.target.value as 'all' | EvidenceStatus)}><option value="all">All statuses</option><option value="active">Active</option><option value="supported">Supported</option><option value="mixed">Mixed</option><option value="contradicted">Contradicted</option><option value="falsified">Falsified</option><option value="retracted">Retracted</option></select></label>
      </div>
      {filtered.length === 0 ? <div className="empty-state"><strong>No evidence matches these filters</strong><span>Adjust filters or register governed evidence.</span></div> : (
        <div className="evidence-list">{filtered.map((record) => (
          <article key={record.id} className="evidence-card">
            ...
            <dl><div><dt>Source</dt><dd>{record.sourceName}</dd></div><div><dt>Identifier</dt><dd>{record.sourceIdentifier}</dd></div><div><dt>Replication</dt><dd>{record.replicationStatus}</dd></div><div><dt>Confidence</dt><dd>{Math.round(record.confidence * 100)}%</dd></div></dl>
...
function FalsificationPage() {
  return (
    <div className="page-stack">
      <PageHeader eyebrow="Disease Reconstruction Lab" title="Falsification Engine" description="A hypothesis is not protected from bad news. Counterexamples, alternative explanations, escape routes and evidence gaps remain first-class records." />
      {falsificationRecords.map((record) => (
        <article key={record.id} className="feature-card wide">
          ...
          <div className="falsification-grid"><div><strong>Counterexample</strong><p>{record.counterexample}</p></div><div><strong>Escape route</strong><p>{record.escapeRoute}</p></div><div><strong>Alternative explanations</strong><ul>{record.alternativeExplanations.map((item) => <li key={item}>{item}</li>)}</ul></div><div><strong>Evidence gaps</strong><ul>{record.evidenceGaps.map((item) => <li key={item}>{item}</li>)}</ul></div></div>
        </article>
      ))}
    </div>
  );
...
function VulnerabilityPage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="Disease Reconstruction Lab" title="Reconstruction Vulnerability Engine" description="Transparent rule-based research model. Scores are synthetic demonstration outputs, never clinical risk scores." />
...
function CurabilityPage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="Disease Reconstruction Lab" title="Curability Index" description="Research classification that separates symptom control, disease modification, remission, reproducible cure, elimination and eradication." />
...
function UpdatesPage() {
  return (
    <div className="page-stack"><PageHeader eyebrow="Disease Reconstruction Lab" title="Research Updates" description="This view reports only data actually registered in the repository." /><div className="empty-state"><strong>No external research feed configured</strong><span>Connect an authorized evidence ingestion source before presenting current publications as live updates.</span></div></div>
  );
}

function SettingsPage() {
  return (
    <div className="page-stack"><PageHeader eyebrow="Disease Reconstruction Lab" title="Settings" description="Environment and governance status for this implementation milestone." /><div className="settings-grid"><article><strong>Environment</strong><span>Development</span></article><article><strong>Datastore</strong><span>Deterministic repository data only</span></article><article><strong>Clinical integrations</strong><span>Not configured</span></article><article><strong>Patient data</strong><span>Not accepted by this milestone</span></article></div></div>
  );
}

function NotFound() {
  return <section className="page-stack"><PageHeader eyebrow="Navigation" title="Route not found" description="This route is not part of the supported ATLAS Health flow." /><Link className="action-link" to="/health">Return to ATLAS Health</Link></section>;
}
