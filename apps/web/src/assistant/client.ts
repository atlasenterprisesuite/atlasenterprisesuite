...
import { serializeAssistantElementContext, type AssistantElementContext } from './elementContext';
...
export async function sendAssistantMessage(input: {
  ...
  const selectedElement = input.elementContext ? serializeAssistantElementContext(input.elementContext) : '';
  const response = await authorizedAtlasFetch('/functions/v1/atlas-copilot?api=chat', {
    method: 'POST',
    headers,
    body: JSON.stringify({
      ...
      context: [serializeAssistantPageContext(pageContext), selectedElement ? `Selected element (structural metadata only):\n${selectedElement}` : ''].filter(Boolean).join('\n\n'),
      intent: 'balanced',
      mode: 'auto',
      message,
      conversation_id: input.conversationId || null,
