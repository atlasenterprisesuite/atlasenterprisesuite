...
const PROFILES: Array<{ value: IntelligenceProfile; label: string }> = [
  { value: 'fast', label: 'Fast' },
  { value: 'balanced', label: 'Balanced' },
  { value: 'deep', label: 'Deep' }
];

function textOf(message: IntelligenceMessage) {
  if (typeof message.content === 'string') return message.content;
  return String(message.content?.text || '');
}

function metaOf(message: IntelligenceMessage) {
...
function providerClass(provider: ProviderReadiness) {
...
export function UnifiedAIChatPage() {
  ...
  const [profile, setProfile] = useState<IntelligenceProfile>('balanced');
  ...
  const verifiedProviders = providers.filter((provider) => provider.state === 'verified');
  ...
  const routeReady = mode === 'auto'
    ...
    : mode === 'council'
      ...
      : selectedProvider?.state === 'verified';
  ...
  useEffect(() => {
    let active = true;
    Promise.all([loadIntelligenceStatus(), listIntelligenceConversations()])
  ...
  async function refreshStatus() {
    try {
      setStatus(await loadIntelligenceStatus());
      setError('');
    } catch (cause) {
  ...
  async function refreshHistory() {
    try {
      setConversations(await listIntelligenceConversations());
    } catch (cause) {
  ...
  async function openConversation(id: string) {
    ...
    setError('');
    try {
      const payload = await getIntelligenceConversation(id);
  ...
  async function submit(event: FormEvent) {
    ...
    setMessages((current) => [...current, { key: optimisticKey, role: 'user', text: message }]);
    setPrompt('');
    setBusy(true);
    setError('');

    try {
      const result = await sendIntelligenceMessage({ message, conversationId, mode, profile });
      setConversationId(result.conversation_id);
      const providersUsed = Array.isArray(result.providers) && result.providers.length
        ? result.providers.join(' + ')
        : result.provider;
      setMessages((current) => [...current, {
        key: result.conversation_id + '-' + Date.now(),
        role: 'assistant',
  ...
  return (
    <section className="page-stack atlas-ai-page">
      <header className="page-header">
        ...
        <h1>ATLAS Unified AI Chat</h1>
        <p>One governed conversation surface for Auto routing, OpenAI, Gemini, Codex Sovereign and Council mode. Provider availability shown here comes from server-verified readiness.</p>
      </header>
      ...
      <div className="atlas-ai-toolbar">
        <div className="atlas-ai-selectors">
          <label>
            <span>AI mode</span>
            <select value={mode} onChange={(event) => setMode(event.target.value as IntelligenceMode)}>
              {MODES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
            </select>
          </label>
          <label>
            <span>Reasoning profile</span>
            <select value={profile} onChange={(event) => setProfile(event.target.value as IntelligenceProfile)}>
      ...
      <div className="atlas-ai-grid">
        ...
        <aside className="atlas-ai-panel atlas-ai-diagnostics">
          ...
          <dl>
            ...
            <div><dt>Council cost policy</dt><dd>{councilConfigured ? 'Pre-authorized' : 'Approval required'}</dd></div>
          </dl>
          <a className="atlas-ai-link" href="https://chatgpt.com" target="_blank" rel="noopener noreferrer">Open in ChatGPT</a>
          <p className="atlas-ai-empty">The external ChatGPT link is optional. ATLAS provider calls use the governed server API, not the ChatGPT website.</p>
        </aside>
      </div>
    </section>
  );
