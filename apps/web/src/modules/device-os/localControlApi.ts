import { authorizedAtlasFetch, getActiveAtlasOrganization, getAtlasAccessToken } from '../../lib/atlasSession';
...
export type AtlasLocalAgent = {
  ...
  public_key_fingerprint: string | null; mtls_status: 'unconfigured'|'pending'|'active'|'expired'|'revoked';\n  mtls_cert_fingerprint_sha256: string | null; mtls_cert_serial: string | null; mtls_cert_expires_at: string | null;\n  realtime_last_connected_at: string | null; installer_version: string | null;\n  last_seen_at: string | null; created_at: string; updated_at: string;
};
...
export async function bindLocalAgentMtls(input: {
  agentId: string; fingerprintSha256: string; serial: string; expiresAt: string;
}) {
  return post<{ok:true;agent:Pick<AtlasLocalAgent,'id'|'mtls_status'|'mtls_cert_fingerprint_sha256'|'mtls_cert_serial'|'mtls_cert_expires_at'>}>(
    'agents.mtls.bind',
    {
...
async function publishRealtimeNudge(input: { organizationId: string; agentId: string; commandId: string }) {
  ...
  try {
    ...
    const body = await response.json().catch(() => null);
    if (!response.ok || body?.ok !== true) return { delivered: 0, state: 'fallback_polling' as const };
...
export async function enqueueLocalDeviceCommand(input: {
  deviceId: string; agentId: string; capability: string; action: string; riskLevel?: 'low'|'medium'; approvalId?: string;
}) {
  const organization = await getActiveAtlasOrganization();
