...
-- Public submission writes only through the atlas-advisory-public Edge Function.
-- Authenticated staff operate through organization-scoped RPCs.
...
create table if not exists public.advisory_launch_intakes (
  ...
  service_id text not null default 'business-launch-360' check (service_id = 'business-launch-360'),
  ...
  business_stage text,
  goals text,
  status text not null default 'new' check (status in ('new','quoted','converted','invoiced','closed')),
...
create policy advisory_launch_intakes_read on public.advisory_launch_intakes
...
drop trigger if exists advisory_launch_intakes_updated_at on public.advisory_launch_intakes;
create trigger advisory_launch_intakes_updated_at
before update on public.advisory_launch_intakes
for each row execute function public.set_updated_at();

drop trigger if exists advisory_launch_intakes_audit on public.advisory_launch_intakes;
...
begin
  ...
  set quote_amount = round(p_amount::numeric, 2),
      ...
      status = 'quoted',
      updated_at = now()
  where id = p_intake_id and org_id = v_org and status not in ('invoiced','closed');

  if not found then raise exception 'Launch intake not available'; end if;
  return query select i.* from public.advisory_launch_intakes i where i.id = p_intake_id and i.org_id = v_org;
end
$$;
...
begin
  ...
  if v_intake.quote_amount is null or v_intake.quote_amount <= 0 then
    raise exception 'Quote required before conversion';
  end if;
  if v_intake.status = 'closed' then raise exception 'Closed intake cannot be converted'; end if;
  ...
  if v_client is null then
    insert into public.advisory_clients(
      ...
      v_org,
      v_intake.firm_id,
      coalesce(nullif(trim(coalesce(v_intake.business_name,'')), ''), v_intake.full_name),
      case when nullif(trim(coalesce(v_intake.business_name,'')), '') is null then 'person' else 'business' end,
      v_intake.email,
      v_intake.phone,
      'active',
      v_user,
  ...
  if v_engagement is null then
    insert into public.advisory_engagements(
      org_id, firm_id, client_id, service_id, title, status, owner_id, starts_on, created_by
    ) values (
      v_org,
      v_intake.firm_id,
      v_client,
      'business-launch-360',
      'Business Launch 360 · ' || coalesce(nullif(trim(coalesce(v_intake.business_name,'')), ''), v_intake.full_name),
      'open',
      v_user,
      current_date,
      v_user
  ...
  set advisory_client_id = v_client,
      ...
      status = case when invoice_id is not null then 'invoiced' else 'converted' end,
      updated_at = now()
  where id = p_intake_id and org_id = v_org;
...
begin
  ...
  if p_receivable_customer_id is not null and not exists (
    select 1 from public.customers c where c.id = p_receivable_customer_id and c.org_id = v_org
  ) then
...
revoke all on function public.advisory_set_launch_quote(uuid,numeric,text) from public;
...
grant execute on function public.advisory_set_launch_quote(uuid,numeric,text) to authenticated;
...
comment on table public.advisory_launch_intakes is
  'Public Business Launch 360 requests routed to one active Advisory firm. No anonymous direct table access.';
