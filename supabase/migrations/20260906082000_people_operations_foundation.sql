...
create table public.people_assessment_results (
  ...
  created_at timestamptz not null default now(),
  check ((score is null and max_score is null) or (score is not null and max_score is not null and max_score > 0 and score >= 0 and score <= max_score))
);
...
for select to authenticated
using (public.has_identity_permission(org_id, 'payroll.read'));

create policy people_payroll_runs_write on public.people_payroll_runs
for all to authenticated
using (public.has_identity_permission(org_id, 'payroll.write'))
with check (public.has_identity_permission(org_id, 'payroll.write'));
...
using (
  ...
  or (
    public.has_identity_permission(org_id, 'payroll.self')
    and exists (
      select 1
      from public.employees e
      where e.id = employee_id
...
for select to authenticated
using (public.has_identity_permission(org_id, 'hr.read'));
...
begin
  ...
  if (
    ...
    and new.status in ('approved', 'locked')
  ) then
