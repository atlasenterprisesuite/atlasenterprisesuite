...
begin
  ...
  if v_submission.subject_user_id = p_actor_user_id then
    raise exception 'authorization_denied';
  end if;
...
comment on function public.atlas_ride_compliance_review_transition(uuid,uuid,uuid,text,text) is
  'Atomically transitions ATLAS Ride profile-photo review state and writes the matching review audit event under row locks; self-review is denied.';
