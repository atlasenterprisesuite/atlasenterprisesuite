...
async function safetyId(context={}){const raw=`${context.organization_id||'org'}:${context.user_id||'user'}`;const bytes=new TextEncoder().encode(raw);const digest=await crypto.subtle.digest('SHA-256',bytes);return `atlas_${Array.from(new Uint8Array(digest)).map(b=>b.toString(16).padStart(2,'0')).join('').slice(0,32)}`;}
