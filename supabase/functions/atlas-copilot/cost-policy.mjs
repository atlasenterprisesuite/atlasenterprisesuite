export function evaluateIntelligenceCostPolicy({mode='auto',providers=[],policy={}}={}){
  ...
  if(mode==='council'){
    ...
    if(providers.every(provider=>zero.has(provider)))return {decision:'allow',reason:'zero_cost_council',providers:[...providers]};
  ...
  if(providers.every(provider=>zero.has(provider)))return {decision:'allow',reason:'zero_cost_provider',providers:[...providers]};
