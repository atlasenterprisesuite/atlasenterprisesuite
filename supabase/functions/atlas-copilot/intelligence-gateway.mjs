import {DEFAULT_AI_DATA_POLICY,shouldStoreProviderResponse} from './ai-data-policy.mjs';
...
export function normalizeIntelligenceRequest(input={}){
  ...
  const store_provider_response=input.store_provider_response===true?true:input.store_provider_response===false?false:null;
  return Object.freeze({module,intent,mode,message,capabilities_requested,conversation_id:input.conversation_id?String(input.conversation_id):null,store_provider_response,client_metadata:input.client_metadata&&typeof input.client_metadata==='object'?structuredClone(input.client_metadata):{},legacy_context:input.legacy_context?String(input.legacy_context).slice(0,12000):''});
}
...
export function createIntelligenceGateway({router,provider,registry,council,store,costPolicy,toolGateway,clock=Date.now}={}){
  ...
  return Object.freeze({async execute({context,request}){
    ...
    const dataPolicy=store.getDataPolicy?await store.getDataPolicy({context:principal}):DEFAULT_AI_DATA_POLICY;
    const providerStore=shouldStoreProviderResponse({policy:dataPolicy,module:normalized.module,perCallStore:normalized.store_provider_response});
    ...
    try{
      ...
      if(route.mode==='council'){
        ...
        result=await council.execute({providerIds:route.providers,context:principal,route,instructions,input:history,max_output_tokens:3000,store:providerStore});
      }else{
        ...
        result=await adapter.execute({context:principal,route,instructions,input:history,max_output_tokens:3000,store:providerStore});
      }
      ...
      const routing={mode:route.mode,providers:route.providers,profile:route.profile,fallback_used:route.fallback_used,reason:route.reason,provider_storage:providerStore?'enabled':'disabled'};
