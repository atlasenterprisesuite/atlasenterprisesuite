...
describe('ATLAS Work fail-closed production verification', () => {
  ...
  it('checks Work through the authorized production verifier and exact-version route set', () => {
    ...
    expect(authorizedVerifier).toContain('statusRouteOk');
    expect(authorizedVerifier).toContain('canonicalModuleRoutesOk');
    expect(authorizedVerifier).toContain('workRoutesOk');
    expect(authorizedVerifier).toContain('criticalNetworkRoutesOk');
    expect(authorizedVerifier).toContain('productionCommitVerified');
