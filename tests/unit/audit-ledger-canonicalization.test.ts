...
describe('ATLAS Audit Ledger canonical hashing', () => {
  ...
  it('fails closed instead of silently dropping non-JSON values', () => {
    ...
    expect(() => canonicalizeJson(sparse)).toThrow('audit_ledger_invalid_json');
    const symbolKey = Symbol('hidden');
    const withSymbol: Record<PropertyKey, unknown> = { safe: true };
    withSymbol[symbolKey] = 'secret';
    expect(() => canonicalizeJson(withSymbol)).toThrow('audit_ledger_invalid_json');
