...
describe('ATLAS Chat Core', () => {
  ...
  it('keeps the legacy chat route only as a compatibility redirect and removes the module card', () => {
  ...
  it('does not classify the compatibility redirect as a canonical production module route', () => {
    expect(read(productionContractPath)).not.toContain('"/connect/chat"');
    expect(read(authorizedVerifierPath)).not.toContain("'/connect/chat'");
  });
});
