...
const atomicMigrationPath = resolve(root, 'supabase/migrations/20260912_ride_profile_photo_review_atomicity.sql');
...
describe('ATLAS Ride compliance schema contract', () => {
  ...
  it('transitions review state atomically in PostgreSQL', () => {
    ...
    expect(atomicSql.match(/for update/gi)?.length).toBeGreaterThanOrEqual(2);
